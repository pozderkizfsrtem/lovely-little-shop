...
const makeSchema = (t: Translation) =>
  z.object({
    firstName: z.string().trim().min(1, t.checkout.errFirstName).max(60),
    lastName: z.string().trim().min(1, t.checkout.errLastName).max(60),
    email: z.string().trim().email(t.checkout.errEmail).max(255),
    phone: z
      .string()
      .trim()
      .min(9, t.checkout.errPhone)
      .max(20)
      .regex(/^[+\d\s-]+$/, t.checkout.errPhoneFormat),
    paczkomat: z.string().trim().min(3, t.checkout.errLocker).max(120),
  });
...
const Checkout = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const res = makeSchema(t).safeParse(form);
    if (!res.success) {
    ...
    try {
      ...
      if (error || !data?.success) {
        throw new Error(error?.message || data?.error || t.checkout.sendError);
      }
  ...
  if (count === 0) {
    return (
      <div className="max-w-2xl mx-auto px-6 py-20 text-center">
        <p className="text-gold text-xs uppercase tracking-[0.3em] mb-4">— {t.checkout.eyebrow}</p>
        <h1 className="font-display text-4xl mb-6">{t.checkout.emptyTitle}</h1>
        <Link to="/">
          <Button className="rounded-full">{t.backToShop}</Button>
        </Link>
      </div>
    );
  }
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-2xl mx-auto px-6 py-20">
        ...
        <h1 className="font-display text-5xl md:text-6xl leading-tight mb-10">
          {t.checkout.titlePrefix} <span className="italic text-gold">{t.checkout.titleAccent}</span>
        </h1>
            ...
            {grouped.map((pid) => {
              ...
              return (
                <li key={pid} className="flex justify-between">
                  ...
                  <span className="text-gold whitespace-nowrap">{unit * qty} {t.currency}</span>
                </li>
              );
            })}
          </ul>
        ...
        <form onSubmit={onSubmit} className="space-y-5">
          ...
          <div>
            <Label htmlFor="paczkomat">{t.checkout.locker}</Label>
            <div className="mt-1.5 flex gap-2">
              <Input
                id="paczkomat"
                placeholder={t.checkout.lockerPlaceholder}
          ...
          <div className="flex items-center justify-between pt-4">
            ...
            >
              {submitting ? t.checkout.submitting : `${t.checkout.submit} · ${total} ${t.currency}`}
            </Button>
          </div>
        </form>
      </div>
      ...
      <Dialog open={mapOpen} onOpenChange={setMapOpen}>
        <DialogContent className="max-w-4xl w-[95vw] h-[85vh] p-0 overflow-hidden flex flex-col">
          <DialogHeader className="px-6 pt-5 pb-3 border-b border-border/50">
            <DialogTitle>{t.pickParcelLocker}</DialogTitle>
          </DialogHeader>
